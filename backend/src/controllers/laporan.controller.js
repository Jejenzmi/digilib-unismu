...
exports.statistik = async (req, res) => {
  try {
    ...
    const fakId       = isFakAdmin ? req.user.fakultasId : null;
    // Scope peminjaman jika pustakawan_fakultas
    const pWhere      = fakId ? { user: { fakultasId: fakId } } : {};
    const pDipinjam   = { ...pWhere, status: "dipinjam" };
    const pTerlambat  = { ...pWhere, status: "terlambat" };
    const pKembali    = { ...pWhere, status: "dikembalikan" };
    ...
    const [
      ...
      prisma.user.count({ where: { isActive: true, ...(fakId && { fakultasId: fakId }) } }),
      prisma.peminjaman.count({ where: pWhere }),
      prisma.skripsi.count({ where: { isApproved: true } }),
      prisma.jurnal.count({ where: { isActive: true } }),
      prisma.peminjaman.count({ where: pDipinjam }),
      prisma.peminjaman.count({ where: pTerlambat }),
      prisma.peminjaman.count({ where: pKembali }),
      ...
      prisma.peminjaman.aggregate({ _sum: { denda: true }, where: pWhere }),
    ]);
    return ok(res, {
      totalBuku, totalAnggota, totalPeminjaman, totalSkripsi, totalJurnal,
      ...
      ...(isFakAdmin && { scopedFakultasId: fakId, scopedFakultasNama: req.user.fakultas?.nama }),
    });
