...
exports.getAll = async (req, res) => {
  try {
    ...
    const isFakAdmin  = req.user.role === "pustakawan_fakultas";
    // Pustakawan fakultas hanya bisa lihat anggota di fakultasnya
    const scopedFakultasId = isFakAdmin ? req.user.fakultasId : (fakultasId ? +fakultasId : undefined);
    const where = {
      ...(role                  && { role }),
      ...(scopedFakultasId      && { fakultasId: scopedFakultasId }),
...
exports.getOne = async (req, res) => {
  try {
    ...
    const isFakAdmin = req.user.role === "pustakawan_fakultas";
    const sameFakult = isFakAdmin && req.user.fakultasId &&
                       user.fakultasId === req.user.fakultasId;
    if (!isAdmin && !isFakAdmin && req.user.id !== user.id) return fail(res, "Akses ditolak.", 403);
    if (isFakAdmin && req.user.id !== user.id && !sameFakult) return fail(res, "Akses ditolak. Anggota di luar fakultas Anda.", 403);
...
exports.create = async (req, res) => {
  try {
    ...
    // Hanya Pustakawan Universitas yang boleh membuat akun pustakawan
    if (["pustakawan_universitas","pustakawan_fakultas"].includes(role) && req.user.role !== "pustakawan_universitas")
      return fail(res, "Hanya Pustakawan Universitas yang dapat membuat akun pustakawan.", 403);
    // pustakawan_fakultas wajib ada fakultasId
    if (role === "pustakawan_fakultas" && !fakultasId)
      return fail(res, "Pustakawan Fakultas wajib ditentukan fakultasnya.", 400);
    ...
    // Kirim email verifikasi jika email tersedia (fire-and-forget)
    if (user.email && env.smtpHost) {
      const token = emailSvc.generateVerificationToken();
      emailSvc.sendVerificationEmail({ to: user.email, name: user.name, token }).catch(() => {});
    }
...
exports.update = async (req, res) => {
  try {
    ...
    const isAdmin    = ["pustakawan_universitas","pustakawan_fakultas"].includes(req.user.role);
    const isFakAdmin = req.user.role === "pustakawan_fakultas";
    if (!isAdmin && !isFakAdmin && req.user.id !== id) return fail(res, "Akses ditolak.", 403);
    if (isFakAdmin && req.user.id !== id) {
      const target = await prisma.user.findUnique({ where: { id } });
      if (!target || target.fakultasId !== req.user.fakultasId)
        return fail(res, "Akses ditolak. Anggota di luar fakultas Anda.", 403);
    }
...
exports.updateAvatar = async (req, res) => {
  try {
    ...
    const id      = +req.params.id;
    const isAdmin = ["pustakawan_universitas", "pustakawan_fakultas", "pustakawan_fakultas"].includes(req.user.role);
...
exports.remove = async (req, res) => {
  try {
    ...
    if (!user)              return fail(res, "Pengguna tidak ditemukan.", 404);
    if (user.role==="pustakawan_universitas") return fail(res, "Akun Pustakawan Universitas tidak dapat dihapus.");
    if (["pustakawan_fakultas","pustakawan_fakultas"].includes(user.role) && req.user.role !== "pustakawan_universitas")
      return fail(res, "Hanya Kepala Perpustakaan yang dapat menonaktifkan akun pustakawan.", 403);
