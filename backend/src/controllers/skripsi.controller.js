...
exports.create = async (req, res) => {
  try {
    ...
    const filePath = req.file
      ? await storage.uploadFile(req.file.path, `skripsi/${req.file.filename}`, "application/pdf")
      : null;
...
exports.remove = async (req, res) => {
  try {
    const s = await prisma.skripsi.findUnique({ where: { id: +req.params.id } });
    if (!s) return fail(res, "Skripsi tidak ditemukan.", 404);
    if (s.filePath) await storage.deleteFile(s.filePath).catch(() => {});
...
exports.download = async (req, res) => {
  try {
    ...
    await storage.streamFile(s.filePath, res, `${s.authorName} - ${s.title}`);
