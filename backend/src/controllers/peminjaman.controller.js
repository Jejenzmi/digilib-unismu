...
exports.getAll = async (req, res) => {
  try {
    ...
    const where   = {
      ...
      // User biasa: hanya peminjaman sendiri
      ...(!isAdmin && !isFakAdmin && { userId: req.user.id }),
      // Pustakawan pusat: bisa filter by userId
      ...(isAdmin && userId && { userId: +userId }),
      // Pustakawan fakultas: hanya anggota di fakultasnya
      ...(isFakAdmin && {
        user: { fakultasId: req.user.fakultasId }
      }),
      ...(isFakAdmin && userId && { userId: +userId }),
    };
...
exports.getOne = async (req, res) => {
  try {
    ...
    const isAdmin    = ["pustakawan_universitas","pustakawan_fakultas"].includes(req.user.role);
    const isFakAdmin = req.user.role === "pustakawan_fakultas";
    const sameUser   = p.userId === req.user.id;
    const sameFakult = isFakAdmin && req.user.fakultasId &&
                       p.user?.fakultasId === req.user.fakultasId;
    if (!isAdmin && !isFakAdmin && !sameUser)      return fail(res, "Akses ditolak.", 403);
    if (isFakAdmin && !sameUser && !sameFakult)    return fail(res, "Akses ditolak. Anggota di luar fakultas Anda.", 403);
...
exports.pinjam = async (req, res) => {
  try {
    ...
    // Kirim notifikasi email dengan token (fire-and-forget)
    if (req.user.email && result.pinjam?.token) {
      emailSvc.sendBorrowNotification({
        to: req.user.email, name: req.user.name,
        bookTitle: result.bookTitle, token: result.pinjam.token,
        expiryDate: result.pinjam.tanggalKembali,
      }).catch(() => {});
    }
